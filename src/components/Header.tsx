    ...
    <header className="sticky top-0 z-40 w-full bg-cabinet/80 backdrop-blur-sm">
      <nav className="max-w-6xl mx-auto px-4">
        <div className="flex items-center justify-between h-14 border-b-2 border-screen">
          <Link href="/" className="font-arcade text-lg text-cyan hover:text-magenta transition-colors">
            Simonlinn.com
          </Link>
          ...
          <div className="md:hidden">
            <Sheet open={isOpen} onOpenChange={setIsOpen}>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="text-pixel hover:text-cyan">
                  <Menu className="h-6 w-6" />
                  <span className="sr-only">Open menu</span>
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="bg-cabinet/95 border-l-2 border-screen w-[250px] crt p-0">
                <SheetHeader className="flex flex-row justify-between items-center p-4 border-b-2 border-screen space-y-0">
                    <SheetTitle className="font-arcade text-lg text-cyan">MENU</SheetTitle>
                    <SheetClose asChild>
                         <Button variant="ghost" size="icon" className="text-pixel hover:text-cyan">
                            <X className="h-6 w-6" />
